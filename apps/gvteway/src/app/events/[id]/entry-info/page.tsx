        ...
        <Grid cols={3} gap={8}>
          <Stack className="col-span-2" gap={6}>
            <Card className="p-6">
              ...
              <Grid cols={2} gap={6}>
                <Stack className="p-4 bg-gray-50 rounded">
                  <Label className="text-gray-500">Doors Open</Label>
                  <H3>{info.doors_open}</H3>
                </Stack>
                <Stack className="p-4 bg-gray-50 rounded">
                  <Label className="text-gray-500">Show Starts</Label>
            ...
            <Card className="p-6">
              ...
              <Stack gap={4}>
                {info.entry_gates.map((gate, index) => (
                  <Stack key={index} className="p-4 border border-gray-200 rounded">
                    <Stack direction="horizontal" className="justify-between items-start">
                      <Stack>
                        <Body className="font-bold">{gate.name}</Body>
                        <Body className="text-gray-600">{gate.location}</Body>
                      </Stack>
            ...
            <Card className="p-6">
              ...
              <Grid cols={2} gap={6} className="mt-6">
                ...
                <Stack>
                  <H3 className="mb-3 text-red-600">PROHIBITED ITEMS</H3>
                  <Stack gap={2}>
                    {info.prohibited_items.map((item, index) => (
                      <Stack key={index} direction="horizontal" gap={2} className="items-center">
                        <Body className="text-red-600">✕</Body>
                        <Body>{item}</Body>
                      </Stack>
                    ))}
                  </Stack>
            ...
            {info.parking_info.available && (
              <Card className="p-6">
                    ...
                    <Stack key={index} className="p-4 border border-gray-200 rounded">
                      <Stack direction="horizontal" className="justify-between">
                        <Stack>
                          ...
                          <Body className="text-gray-600 text-sm">{lot.address}</Body>
                        </Stack>
                ...
                {info.parking_info.tips && (
                  <Body className="mt-4 text-gray-600 text-sm">
                    💡 {info.parking_info.tips}
                  </Body>
                )}
              </Card>
            ...
            <Card className="p-6">
              ...
              <Stack gap={4}>
                {info.transit_info.subway && (
                  <Stack className="p-4 bg-gray-50 rounded">
                    <Label className="text-gray-500">Subway</Label>
                ...
                {info.transit_info.rideshare_dropoff && (
                  <Stack className="p-4 bg-gray-50 rounded">
                    <Label className="text-gray-500">Rideshare Drop-off</Label>
              ...
              <Card className="p-6">
                ...
                <Stack gap={4}>
                  {info.faq.map((item, index) => (
                    <Stack key={index} className="pb-4 border-b border-gray-200 last:border-0">
                      <Body className="font-bold mb-2">{item.question}</Body>
                      <Body className="text-gray-600">{item.answer}</Body>
                    </Stack>
                  ))}
                </Stack>
              </Card>
          ...
          <Stack gap={6}>
            <Card className="p-6 bg-black text-white sticky top-6">
              ...
              <Body className="text-gray-300">{info.venue_address}</Body>
              <Body className="text-gray-300">{info.venue_city}</Body>
              <Button
                variant="outline"
              ...
              <Card className="p-6">
                ...
                <Stack gap={3}>
                  {info.age_restriction && (
                    <Stack>
                      <Label className="text-gray-500">Age Restriction</Label>
                      <Body>{info.age_restriction}</Body>
                    </Stack>
                  )}
                  {info.dress_code && (
                    <Stack>
                      <Label className="text-gray-500">Dress Code</Label>
