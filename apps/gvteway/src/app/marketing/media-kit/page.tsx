...
export default function MediaKitPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "Distributed": return "text-blue-600";
    ...
    <UISection className="min-h-screen bg-white">
      <Container className="py-8">
        <Stack gap={8}>
          <Stack gap={2}>
            <H1>MEDIA KIT</H1>
            <Body className="text-gray-600">Press materials and media asset distribution</Body>
          </Stack>
                ...
                <Grid cols={3} gap={4}>
                  {mockAssets.map((asset) => (
                    <Card key={asset.id} className="border-2 border-black p-4">
                      <Stack gap={3}>
                        <Card className="h-24 bg-gray-100 flex items-center justify-center">
                        ...
                        <Stack direction="horizontal" className="justify-between">
                          <Label className="text-gray-500">{asset.format}</Label>
                          <Label className="text-gray-500">{asset.size}</Label>
                        </Stack>
                  ...
                  <Card key={release.id} className="border-2 border-black p-6">
                    <Grid cols={4} gap={4} className="items-center">
                      <Stack gap={1}>
                        ...
                        <Label className="text-gray-500">{release.event}</Label>
                      </Stack>
                      <Label className="text-gray-600">{release.date}</Label>
                      <Stack direction="horizontal" gap={2}>
                        <Label className={getStatusColor(release.status)}>{release.status}</Label>
                        {release.downloads > 0 && <Label className="text-gray-500">{release.downloads} downloads</Label>}
                      </Stack>
                      <Stack direction="horizontal" gap={2}>
                      ...
                      {[
                        ...
                        <Card key={idx} className="p-3 border border-gray-200">
                          <Stack direction="horizontal" className="justify-between">
                            ...
                            <Label className="text-gray-500">{list.count} contacts</Label>
                          </Stack>
                        </Card>
                      ))}
                    </Grid>
          ...
          <Card className="border-2 border-black p-6">
            <Stack gap={4}>
              ...
              <Body className="text-gray-600">Get all media assets, press releases, and fact sheets in one download.</Body>
        ...
        <ModalBody>
          {selectedAsset && (
            <Stack gap={4}>
              ...
              <Grid cols={2} gap={4}>
                <Stack gap={1}><Label className="text-gray-500">Type</Label><Badge variant="outline">{selectedAsset.type}</Badge></Stack>
                <Stack gap={1}><Label className="text-gray-500">Format</Label><Label>{selectedAsset.format}</Label></Stack>
              </Grid>
              <Grid cols={2} gap={4}>
                <Stack gap={1}><Label className="text-gray-500">Size</Label><Label>{selectedAsset.size}</Label></Stack>
                <Stack gap={1}><Label className="text-gray-500">Updated</Label><Label>{selectedAsset.lastUpdated}</Label></Stack>
              </Grid>
              {selectedAsset.event && <Stack gap={1}><Label className="text-gray-500">Event</Label><Label>{selectedAsset.event}</Label></Stack>}
            </Stack>
          )}
        </ModalBody>
        <ModalFooter>
          ...
          <Stack gap={4}>
            <Card className="p-8 border-2 border-dashed border-gray-300 text-center">
              <Stack gap={2}>
                ...
                <Label className="text-gray-500">Drag and drop files here</Label>
