...
export default function AntiScalpingPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "Under Review": case "Flagged": return "text-yellow-600";
      ...
      <Container className="py-8">
        <Stack gap={8}>
          <Stack gap={2}>
            ...
            <Body className="text-gray-600">Protect ticket sales from scalpers and bots</Body>
          </Stack>
          ...
          <Tabs>
                ...
                <TableBody>
                  {mockFlagged.map((txn) => (
                    <TableRow key={txn.id}>
                      ...
                      <TableCell><Label className="text-gray-500">{txn.buyerEmail}</Label></TableCell>
            ...
            <TabPanel active={activeTab === "rules"}>
              <Stack gap={4}>
                {mockRules.map((rule) => (
                  <Card key={rule.id} className={`border-2 p-4 ${rule.enabled ? "border-black" : "border-gray-200"}`}>
              ...
              <Grid cols={2} gap={6}>
                <Card className="border-2 border-black p-6">
                  <Stack gap={4}>
                    ...
                    <Grid cols={2} gap={4}>
                      <Stack gap={1}><Label className="text-gray-500">Total Flagged</Label><Label className="font-mono text-2xl">247</Label></Stack>
                      <Stack gap={1}><Label className="text-gray-500">Blocked</Label><Label className="font-mono text-2xl text-red-600">89</Label></Stack>
                      <Stack gap={1}><Label className="text-gray-500">Cleared</Label><Label className="font-mono text-2xl text-green-600">142</Label></Stack>
                      <Stack gap={1}><Label className="text-gray-500">Pending</Label><Label className="font-mono text-2xl text-yellow-600">16</Label></Stack>
                    </Grid>
                  </Stack>
                </Card>
                <Card className="border-2 border-black p-6">
      ...
      <Modal open={!!selectedTransaction} onClose={() => setSelectedTransaction(null)}>
        ...
        <ModalBody>
          {selectedTransaction && (
            <Stack gap={4}>
              <Grid cols={2} gap={4}>
                <Stack gap={1}><Label className="text-gray-500">Order ID</Label><Label className="font-mono">{selectedTransaction.orderId}</Label></Stack>
                <Stack gap={1}><Label className="text-gray-500">Risk Score</Label><Label className={getRiskColor(selectedTransaction.riskScore)}>{selectedTransaction.riskScore}%</Label></Stack>
              </Grid>
              <Stack gap={1}><Label className="text-gray-500">Event</Label><Label>{selectedTransaction.eventName}</Label></Stack>
              <Stack gap={1}><Label className="text-gray-500">Buyer</Label><Label>{selectedTransaction.buyerEmail}</Label></Stack>
              <Grid cols={2} gap={4}>
                <Stack gap={1}><Label className="text-gray-500">Quantity</Label><Label className="font-mono">{selectedTransaction.quantity}</Label></Stack>
                <Stack gap={1}><Label className="text-gray-500">Timestamp</Label><Label>{selectedTransaction.timestamp}</Label></Stack>
              </Grid>
              <Stack gap={1}><Label className="text-gray-500">Flag Reason</Label><Badge variant="outline">{selectedTransaction.flagReason}</Badge></Stack>
              <Stack gap={1}><Label className="text-gray-500">Current Status</Label><Label className={getStatusColor(selectedTransaction.status)}>{selectedTransaction.status}</Label></Stack>
            </Stack>
          )}
        </ModalBody>
        <ModalFooter>
          <Button variant="outline" onClick={() => setSelectedTransaction(null)}>Close</Button>
          <Button variant="outline" className="text-green-600">Clear</Button>
          <Button variant="solid" className="bg-red-600">Block</Button>
        </ModalFooter>
      </Modal>
    </UISection>
  );
