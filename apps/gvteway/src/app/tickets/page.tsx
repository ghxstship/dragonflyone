    ...
    <PageLayout
      ...
      header={
        <Navigation
          ...
          cta={<Button variant="outline" size="sm" onClick={() => router.push('/profile')}>PROFILE</Button>}
        >
          <Link href="/" className="font-heading text-body-sm uppercase tracking-widest hover:text-ink-600">Home</Link>
          <Link href="/events" className="font-heading text-body-sm uppercase tracking-widest hover:text-ink-600">Events</Link>
      ...
      <SectionLayout background="black">
        <Container size="lg">
          <Stack gap={8}>
            ...
            {isLoading ? (
              <Container className="flex justify-center py-spacing-12">
                <LoadingSpinner size="lg" text="Loading tickets..." />
              </Container>
            ) : tickets && tickets.length > 0 ? (
              <Stack gap={6}>
                {tickets.map((ticket) => (
                  <Card key={ticket.id} className="border-2 border-ink-800 p-spacing-6 bg-black">
                    <Stack gap={4}>
                      <Stack gap={2} direction="horizontal" className="justify-between items-start">
                        <Stack gap={3}>
                          <Stack gap={1}>
                            <H3 className="text-white">{ticket.event?.name || 'Event'}</H3>
                            <Body className="text-ink-600">{ticket.event?.venue || 'Venue'}</Body>
                          </Stack>
                          <Grid cols={2} gap={2}>
                            <Stack gap={1} direction="horizontal">
                              <Body className="text-ink-500 text-body-sm">Date:</Body>
                            ...
                            <Stack gap={1} direction="horizontal">
                              <Body className="text-ink-500 text-body-sm">Section:</Body>
                              <Body className="text-white text-body-sm">{ticket.ticket_type?.name || 'General'}</Body>
                            </Stack>
                            <Stack gap={1} direction="horizontal">
                              <Body className="text-ink-500 text-body-sm">Seat:</Body>
                              <Body className="text-white text-body-sm">{ticket.seat_number || 'General Admission'}</Body>
                            </Stack>
                            <Stack gap={1} direction="horizontal">
                              <Body className="text-ink-500 text-body-sm">Ticket ID:</Body>
                        ...
                        <Stack gap={3} className="text-right items-end">
                          ...
                          <Card className="w-spacing-32 h-spacing-32 border-2 border-white bg-white p-spacing-2">
                            <Body className="w-full h-full bg-black/10 flex items-center justify-center text-micro-xs font-mono">
                              {ticket.qr_code}
                            </Body>
                          </Card>
                        </Stack>
