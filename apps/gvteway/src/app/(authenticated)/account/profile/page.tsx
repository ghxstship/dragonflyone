          ...
          <Card className="p-6">
            <Box className="flex items-start gap-6 mb-6">
              ...
              <Box className="flex-1">
                {isEditing ? (
                  ...
                  <><Body className="font-weight-bold">{profile.name}</Body><Body className="text-text-muted">{profile.email}</Body></>
                )}
              </Box>
            ...
            <Box className="border-t border-border pt-6">
              ...
              <Grid cols={2} gap={4} className="grid-cols-1 md:grid-cols-2 mt-4">
                <Box><Body size="sm" className="text-text-muted mb-1">Email</Body>{isEditing ? <Input value={formData.email || ""} onChange={(e) => setFormData({ ...formData, email: e.target.value })} /> : <Box className="flex items-center gap-2"><Mail className="size-4 text-text-muted" /><Body>{profile.email}</Body></Box>}</Box>
                <Box><Body size="sm" className="text-text-muted mb-1">Phone</Body>{isEditing ? <Input value={formData.phone || ""} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} /> : <Box className="flex items-center gap-2"><Phone className="size-4 text-text-muted" /><Body>{profile.phone}</Body></Box>}</Box>
                <Box><Body size="sm" className="text-text-muted mb-1">Location</Body>{isEditing ? <Input value={formData.location || ""} onChange={(e) => setFormData({ ...formData, location: e.target.value })} /> : <Box className="flex items-center gap-2"><MapPin className="size-4 text-text-muted" /><Body>{profile.location}</Body></Box>}</Box>
              </Grid>
            </Box>
