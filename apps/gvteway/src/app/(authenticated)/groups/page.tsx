    ...
    content: (
      <Section>
        <Box className="flex gap-4 items-center mb-6">
          <Box className="relative flex-1 max-w-md"><Search className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-text-muted" /><Input placeholder="Search groups..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10" /></Box>
        </Box>
        {filtered.length === 0 ? (
          <Card className="p-8 text-center"><Users className="size-12 text-text-disabled mx-auto mb-4" /><Body className="font-weight-medium mb-2">No groups yet</Body><Body className="text-text-muted mb-4">Create or join a group</Body><Button variant="solid" icon={<Plus className="size-4" />} iconPosition="left">Create Group</Button></Card>
        ) : (
