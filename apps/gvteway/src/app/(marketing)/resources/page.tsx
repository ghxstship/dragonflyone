...
const PLATFORM_TEMPLATES = getTemplatesByPlatform('gvteway');
const PLATFORM_GUIDES = getGuidesByPlatform('gvteway');
const PLATFORM_VIDEOS = getVideosByPlatform('gvteway');
const UPCOMING_WEBINARS = getUpcomingWebinars();

const RESOURCE_CATEGORIES = [
  { id: "guides", icon: <Book className="size-8" />, title: "Guides", description: `${PLATFORM_GUIDES.length} step-by-step tutorials to help you master GVTEWAY and ticketing.` },
  { id: "templates", icon: <FileText className="size-8" />, title: "Templates", description: `${PLATFORM_TEMPLATES.length} ready-to-use templates for marketing, ticketing, and guest management.` },
...
const FEATURED_TEMPLATES = getFeaturedTemplates().filter(t => t.platform === 'gvteway' || t.platform === 'all').slice(0, 4).map(t => ({
  id: t.id,
  title: t.title,
  type: "Template",
...
const FEATURED_GUIDES_LIST = getFeaturedGuides().filter(g => g.platform === 'gvteway' || g.platform === 'all').slice(0, 2).map(g => ({
  id: g.id,
  title: g.title,
  type: "Guide",
  format: "PDF",
...
const LEARNING_PATHS = [
  { id: "beginner", title: "Getting Started", description: "New to GVTEWAY? Start here.", icon: <GraduationCap className="size-6" />, articles: PLATFORM_GUIDES.filter(g => g.difficulty === 'beginner').length || 8, duration: "2 hours" },
  { id: "intermediate", title: "Advanced Features", description: "Master powerful features.", icon: <Wrench className="size-6" />, articles: PLATFORM_GUIDES.filter(g => g.difficulty === 'intermediate').length || 12, duration: "4 hours" },
  { id: "expert", title: "API & Integrations", description: "Build custom solutions.", icon: <FileText className="size-6" />, articles: PLATFORM_GUIDES.filter(g => g.difficulty === 'advanced').length || 18, duration: "6 hours" },
];
      ...
      sections={[
        {
          ...
          content: (
            <HeroSection
              ...
              description="Everything you need to succeed with GVTEWAY. Guides, templates, video tutorials, and documentation to help you master ticketing and event discovery."
              primaryCta={{
                label: "Browse Guides",
            ...
            <Container size="xl" className="py-20">
              <Stack gap={8}>
                <Stack gap={4} className="text-center items-center">
                  ...
                  <Body className="text-on-dark-muted max-w-2xl">Follow our curated learning paths to master GVTEWAY at your own pace.</Body>
                </Stack>
                ...
                <Grid cols={3} gap={6} className="grid-cols-1 md:grid-cols-3">
                  {LEARNING_PATHS.map((path) => (
                    <Card
                      key={path.id}
                      className="p-6 border-2 border-grey-800 rounded-card pop-card-gvteway group"
            ...
            <Container size="xl" className="py-20">
              <Stack gap={8}>
                <Stack gap={4} className="text-center items-center">
                  ...
                  <Body className="text-on-dark-muted max-w-2xl">Popular templates and guides downloaded by thousands of event professionals.</Body>
                </Stack>
        ...
        },
        {
          ...
          content: (
            <CTABanner
              ...
              description="Our team is here to help you get the most out of GVTEWAY. Schedule a training session or contact support."
              primaryCta={{
