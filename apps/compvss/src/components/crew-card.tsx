  ...
  return (
    <Article variant="bordered" className="p-6">
      <Stack direction="horizontal" className="items-start justify-between">
        <Stack>
          <Label className="font-mono text-mono-xs uppercase tracking-label text-text-muted">{member.id}</Label>
          <H3 className="mt-1 font-display text-h5-md text-white">{member.name}</H3>
          <Body className="mt-1 text-text-secondary">{member.role}</Body>
        </Stack>
      ...
      <Stack direction="horizontal" className="mt-4 items-center justify-between">
        <Body size="sm" className="text-text-muted">{member.department}</Body>
