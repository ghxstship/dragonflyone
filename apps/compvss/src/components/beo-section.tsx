    ...
    switch (section.type) {
        ...
        return (
          <Stack gap={4}>
            ...
            {Array.isArray(content.spaces) && content.spaces.length > 0 && (
              <Stack gap={2}>
                <Body size="sm" className="text-text-muted font-weight-medium">Spaces</Body>
      ...
      case "timeline":
        ...
        return items.length > 0 ? (
          <Stack gap={2}>
            {items.map((item, i) => (
              <Stack key={i} direction="horizontal" gap={4} className="items-start py-2 border-b border-border last:border-0">
                ...
                <Body size="sm" className="text-text-secondary flex-1">{item.description}</Body>
                {item.department && (
          ...
          <Body size="sm" className="text-text-disabled">No timeline items</Body>
        );
        ...
        return (
          <Stack gap={4}>
            {Array.isArray(content.menu_items) && content.menu_items.length > 0 && (
              <Stack gap={2}>
                <Body size="sm" className="text-text-muted font-weight-medium">Menu Items</Body>
                <Stack gap={1}>
                  {(content.menu_items as string[]).map((item, i) => (
                    <Body key={i} size="sm" className="text-text-secondary">- {item}</Body>
                  ))}
                </Stack>
              </Stack>
            )}
            {Array.isArray(content.dietary_requirements) && content.dietary_requirements.length > 0 && (
              <Stack gap={2}>
                <Body size="sm" className="text-text-muted font-weight-medium">Dietary Requirements</Body>
                <Stack direction="horizontal" gap={2} className="flex-wrap">
            ...
            {(!content.menu_items || (content.menu_items as string[]).length === 0) && (
              <Body size="sm" className="text-text-disabled">No catering details</Body>
            )}
          </Stack>
        );
      ...
      case "setup_requirements":
        ...
        return reqItems.length > 0 ? (
          ...
          <Body size="sm" className="text-text-disabled">No requirements listed</Body>
        );

      case "notes":
        return (
          <Body size="sm" className="text-text-secondary whitespace-pre-wrap">
      ...
      <Stack gap={2}>
        {showHeader && (
          <Stack direction="horizontal" gap={2} className="items-center">
            ...
            <Body size="sm" className="font-weight-medium text-text-secondary">{section.title}</Body>
          </Stack>
        )}
        {renderContent()}
      </Stack>
...
function DetailItem({ label, value }: { label: string; value?: string }) {
  return (
    <Stack gap={1}>
      <Body size="sm" className="text-text-disabled">{label}</Body>
      <Body size="sm" className="text-text-secondary">{value || "N/A"}</Body>
    </Stack>
  );
}
