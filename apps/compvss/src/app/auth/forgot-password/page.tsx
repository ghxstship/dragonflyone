        ...
        <div className="max-w-md w-full space-y-8 bg-white p-8 border border-grey-200">
          {submitted ? (
            <div className="text-center space-y-6">
              ...
              <p className="font-body text-body-md text-grey-600">If an account exists with <strong>{email}</strong>, you will receive a password reset link shortly.</p>
              <Link href="/auth/signin" className="inline-block py-3 px-6 bg-black text-white font-heading text-h6-sm uppercase tracking-widest hover:bg-grey-800 transition-colors">Back to Sign In</Link>
            </div>
          ) : (
            <>
              <div className="text-center"><h1 className="font-heading text-h2 uppercase tracking-widest">Reset Password</h1><p className="mt-2 font-body text-body-md text-grey-600">Enter your email address and we&apos;ll send you a link to reset your password.</p></div>
              ...
              <form className="space-y-6" onSubmit={handleSubmit}>
                ...
                <button type="submit" disabled={loading} className="w-full py-3 px-4 bg-black text-white font-heading text-h6-sm uppercase tracking-widest hover:bg-grey-800 disabled:bg-grey-400 transition-colors">{loading ? 'Sending...' : 'Send Reset Link'}</button>
              </form>
