...
const logIncidentSchema = z.object({
  action: z.literal('log_incident'),
  event_id: z.string().uuid(),
  location: z.string(),
  description: z.string().optional(),
  severity: z.string(),
  patient_info: z.record(z.unknown()).optional(),
...
const medicalActionSchema = z.union([logIncidentSchema, assignStaffSchema]);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'log_incident') {
      const { location, description, severity, patient_info, treatment } = validatedData as z.infer<typeof logIncidentSchema>;
    ...
    if (action === 'assign_staff') {
      const { user_id, role, station_id, shift_start, shift_end } = validatedData as z.infer<typeof assignStaffSchema>;
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const { id, status, resolution, transport_required } = validatedData;
