...
const updateAssetSchema = z.object({
  state: z.enum(['available', 'checked_out', 'maintenance', 'retired']).optional(),
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data, error } = await supabase.from('assets').update(payload).eq('id', params.id).select().single();
    if (error) {
      if (error.code === 'PGRST116') return NextResponse.json({ error: 'Asset not found' }, { status: 404 });
      return NextResponse.json({ error: error.message }, { status: 500 });
    }
