...
import { Film, Check, ArrowRight, List } from "lucide-react";
import { Body, Button, Card, Grid, DetailPage, Section, SectionHeader } from "@ghxstship/ui";
import { Stack } from "@ghxstship/ui";

const FEATURES = ["Production planning", "Budget management", "Team coordination", "Timeline tracking"];
const BENEFITS = ["Streamlined workflows", "Budget control", "Better collaboration", "On-time delivery"];
...
export default function ProducersSolutionPage() {
  ...
  const tabs = [{
    ...
    content: (
      <Section>
        <Card className="p-8 mb-8"><Body className="text-h5-md text-grey-300">Specialized tools for producers in the live events industry.</Body></Card>
  ...
  return <DetailPage header={{ kicker: "Solutions", title: "For Producers", description: "Professional tools for producers" }} backButton={{ label: "Solutions", href: "/solutions" }} tabs={tabs} actions={<Button variant="solid" onClick={() => router.push("/demo")}>Get Started</Button>} />;
}
