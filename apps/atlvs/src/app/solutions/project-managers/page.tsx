...
import { ClipboardList, Check, ArrowRight, List } from "lucide-react";
import { Body, Button, Card, Grid, DetailPage, Section, SectionHeader } from "@ghxstship/ui";
import { Stack } from "@ghxstship/ui";

const FEATURES = ["Task management", "Timeline planning", "Resource allocation", "Progress tracking"];
const BENEFITS = ["Clear visibility", "On-time delivery", "Resource optimization", "Stakeholder alignment"];
...
export default function ProjectManagersSolutionPage() {
  ...
  const tabs = [{
    ...
    content: (
      <Section>
        <Card className="p-8 mb-8"><Body className="text-h5-md text-grey-300">Specialized tools for project managers in the live events industry.</Body></Card>
  ...
  return <DetailPage header={{ kicker: "Solutions", title: "For Project Managers", description: "Professional tools for project managers" }} backButton={{ label: "Solutions", href: "/solutions" }} tabs={tabs} actions={<Button variant="solid" onClick={() => router.push("/demo")}>Get Started</Button>} />;
}
