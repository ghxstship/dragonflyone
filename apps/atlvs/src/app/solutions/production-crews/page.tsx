...
import { Users, Check, ArrowRight, List } from "lucide-react";
import { Body, Button, Card, Grid, DetailPage, Section, SectionHeader } from "@ghxstship/ui";
import { Stack } from "@ghxstship/ui";

const FEATURES = ["Crew scheduling", "Call sheets", "Department coordination", "Equipment tracking"];
const BENEFITS = ["Efficient scheduling", "Clear communication", "Team alignment", "Resource management"];
...
export default function ProductionCrewsSolutionPage() {
  ...
  const tabs = [{
    ...
    content: (
      <Section>
        <Card className="p-8 mb-8"><Body className="text-h5-md text-grey-300">Specialized tools for production crews in the live events industry.</Body></Card>
  ...
  return <DetailPage header={{ kicker: "Solutions", title: "For Production Crews", description: "Professional tools for production crews" }} backButton={{ label: "Solutions", href: "/solutions" }} tabs={tabs} actions={<Button variant="solid" onClick={() => router.push("/demo")}>Get Started</Button>} />;
}
