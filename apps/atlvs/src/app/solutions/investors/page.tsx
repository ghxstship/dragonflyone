...
import { TrendingUp, Check, ArrowRight, List } from "lucide-react";
import { Body, Button, Card, Grid, DetailPage, Section, SectionHeader } from "@ghxstship/ui";
import { Stack } from "@ghxstship/ui";

const FEATURES = ["Portfolio tracking", "ROI analytics", "Investment reports", "Deal flow management"];
const BENEFITS = ["Clear visibility", "Data-driven decisions", "Professional reporting", "Streamlined investments"];
...
export default function InvestorsSolutionPage() {
  ...
  const tabs = [{
    ...
    content: (
      <Section>
        <Card className="p-8 mb-8"><Body className="text-h5-md text-grey-300">Specialized tools for investors in the live events industry.</Body></Card>
  ...
  return <DetailPage header={{ kicker: "Solutions", title: "For Investors", description: "Professional tools for investors" }} backButton={{ label: "Solutions", href: "/solutions" }} tabs={tabs} actions={<Button variant="solid" onClick={() => router.push("/demo")}>Get Started</Button>} />;
}
