...
const STEPS: { id: OnboardingStep; label: string; description: string; icon: React.ReactNode }[] = [
  ...
  { id: "role", label: "Role", description: "Your role in the organization", icon: <Briefcase className="size-5" /> },
      ...
      <header className="sticky top-0 z-50 border-b-2 border-black/10 bg-white">
        <Container className="flex h-14 items-center justify-between px-4 sm:h-16 sm:px-6">
          <NextLink href="/" className="transition-transform hover:-translate-y-0.5">
            <Display size="md" className="text-black">ATLVS</Display>
          </NextLink>
        </Container>
      </header>
          ...
          <ScrollReveal animation="slide-up" duration={600}>
            <Card className="border-2 border-black/10 bg-white p-6 shadow-md sm:p-8">
              <Stack gap={6} className="sm:gap-8">
                ...
                {currentStep === "organization" && (
                  <Stack gap={4} className="sm:gap-6">
                    ...
                    <Field label="Your Role">
                      <Input type="text" value={organization.role} onChange={(e) => setOrganization({ ...organization, role: e.target.value })} placeholder="e.g., Production Manager" />
                    </Field>
                ...
                {currentStep === "role" && (
                  <Stack gap={4} className="sm:gap-6">
                    <Body size="sm" className="text-center text-muted">Select your primary role in ATLVS:</Body>
                    <Stack gap={3}>
                      {ATLVS_ROLES.map((role) => (
                        <Stack
                          key={role}
                  ...
                  <Stack gap={4} className="sm:gap-6">
                    ...
                    <Stack gap={4}>
                      <Stack direction="horizontal" gap={3} className="items-center">
                        ...
                        <Label size="sm" className="text-muted">Email notifications for project updates</Label>
                      </Stack>
                      <Stack direction="horizontal" gap={3} className="items-center">
