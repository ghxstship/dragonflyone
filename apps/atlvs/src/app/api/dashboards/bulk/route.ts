...
export const DELETE = apiRoute(
  async (request: NextRequest) => {
    try {
      ...
      const { error } = await supabase.from('dashboard_configs').delete().in('id', validation.data.ids);
  ...
  },
  {
    ...
    audit: { action: 'dashboard_configs:bulk_delete', resource: 'dashboard_configs' },
  }
);
