          ...
          content: (
            <Section>
              <Card className="p-8 text-center">
                ...
                <Body className="text-grey-400 mb-4">This action requires ATLVS Admin or higher role. Current user: {user?.email || "Unknown"}</Body>
          ...
          <Card className="p-6 mb-6 border-primary">
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <Body className="text-grey-400">{formatCurrency(data?.plan.price || 0)}/{data?.plan.interval}</Body>
              </div>
            ...
            <Grid cols={3} gap={4} className="grid-cols-1 md:grid-cols-3">
              <div className="p-3 bg-grey-800 rounded-card">
                <Body size="sm" className="text-grey-400">Bookings</Body>
              ...
              <div className="p-3 bg-grey-800 rounded-card">
                <Body size="sm" className="text-grey-400">Storage</Body>
              ...
              <div className="p-3 bg-grey-800 rounded-card">
                <Body size="sm" className="text-grey-400">Team</Body>
          ...
          <Card className="p-6">
            ...
            {data?.payment_method ? (
              <div className="flex justify-between items-center p-4 bg-grey-800 rounded-card mt-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <Body size="sm" className="text-grey-400">Expires {data.payment_method.exp_month}/{data.payment_method.exp_year}</Body>
                  </div>
                </div>
              ...
              <div className="text-center py-8 bg-grey-800 rounded-card mt-4">
                <CreditCard className="size-8 text-grey-600 mx-auto mb-2" />
                <Body className="text-grey-400">No payment method on file</Body>
                <Button variant="ghost" size="sm" className="mt-2">Add Payment Method</Button>
              </div>
            )}
            <div className="flex items-center gap-2 mt-4 text-grey-400">
        ...
        <Section>
          ...
          {!data?.invoices || data.invoices.length === 0 ? (
            <Card className="p-8 text-center mt-4">
              ...
              <Body className="text-grey-400">No invoices yet</Body>
            </Card>
          ) : (
                ...
                <Card key={invoice.id} className="p-4">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <Body size="sm" className="text-grey-400">{invoice.id}</Body>
                    </div>
          ...
          <Grid cols={3} gap={4} className="grid-cols-1 md:grid-cols-3">
            {PLANS.map((plan) => (
              <Card key={plan.id} className={`p-4 ${plan.popular ? "border-primary" : ""}`}>
                ...
                <Stack direction="horizontal" className="items-baseline"><Body className="text-h3-md font-weight-bold">{formatCurrency(plan.price)}</Body><Body size="sm" className="text-grey-400">/mo</Body></Stack>
                <Stack gap={2} className="mt-4">
                  {plan.features.map((feature, i) => (
                    <Stack key={i} direction="horizontal" gap={2} className="items-center text-body-sm text-grey-400"><Check className="size-3 text-success" />{feature}</Stack>
                  ))}
                </Stack>
