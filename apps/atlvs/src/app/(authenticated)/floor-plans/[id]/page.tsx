            ...
            <div>
              <div className="flex items-center gap-3">
                ...
                {floorPlan.is_template && (
                  <Text className="px-2 py-1 rounded-badge text-body-xs font-weight-medium bg-primary/20 text-primary">
                    Template
                  </Text>
                )}
              </div>
              {floorPlan.space && (
                <Body className="text-body-sm text-muted-foreground">{floorPlan.space.name}</Body>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-3">
          <div className="bg-background border-2 border-border rounded-card overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-muted/30">
              <Text className="text-body-sm font-weight-medium">Floor Plan View</Text>
        ...
        <div className="space-y-6">
          <div className="bg-background border-2 border-border rounded-card p-6">
            ...
            <div className="space-y-3">
              {floorPlan.dimensions && (
                <div>
                  <Body className="text-body-xs text-muted-foreground">Dimensions</Body>
              ...
              {floorPlan.scale && (
                <div>
                  <Body className="text-body-xs text-muted-foreground">Scale</Body>
                  <Body className="text-body-sm font-weight-medium">{floorPlan.scale}px per unit</Body>
                </div>
              )}
              {floorPlan.capacity_by_setup && Object.keys(floorPlan.capacity_by_setup).length > 0 && (
                <div>
                  <Body className="text-body-xs text-muted-foreground mb-2">Capacity</Body>
                  <div className="space-y-1">
                    {Object.entries(floorPlan.capacity_by_setup).map(([setup, capacity]) => (
                      <div key={setup} className="flex justify-between text-body-sm">
                        <Text className="capitalize">{setup.replace(/_/g, ' ')}</Text>
                        <Text className="font-weight-medium">{String(capacity)}</Text>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div>
                <Body className="text-body-xs text-muted-foreground">Objects</Body>
                <Body className="text-body-sm font-weight-medium">{floorPlan.objects?.length || 0} placed</Body>
              </div>
              <div>
              ...
              <div>
                <Body className="text-body-xs text-muted-foreground">Last Modified</Body>
                <Body className="text-body-sm font-weight-medium">
          ...
          {floorPlan.description && (
            <div className="bg-background border-2 border-border rounded-card p-6">
              ...
              <Body className="text-body-sm text-foreground">{floorPlan.description}</Body>
            </div>
          )}
