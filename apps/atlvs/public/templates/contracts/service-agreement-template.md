# PROFESSIONAL SERVICES AGREEMENT

<!-- IMPORT FORMAT: Copy contract details table to CSV for import -->

## Contract Details (Importable)

| field | value |
|-------|-------|
| contract_number | |
| contract_date | |
| project_name | |
| start_date | |
| end_date | |
| client_name | |
| client_contact | |
| client_email | |
| provider_name | |
| provider_contact | |
| provider_email | |
| fee_type | |
| total_amount | |
| payment_terms | |
| status | |

## Agreement Information

| Field | Details |
|-------|---------|
| **Agreement Date** | [Enter Date] |
| **Agreement Number** | PSA-[XXXX] |
| **Project Name** | [Enter Project Name] |
| **Start Date** | [Enter Date] |
| **End Date** | [Enter Date] |

---

## Parties

### Client
**Company Name:** [Enter Company Name]  
**Address:** [Enter Address]  
**City, State, ZIP:** [Enter City, State, ZIP]  
**Contact Name:** [Enter Name]  
**Phone:** [Enter Phone]  
**Email:** [Enter Email]

### Service Provider
**Company Name:** [Enter Company Name]  
**Address:** [Enter Address]  
**City, State, ZIP:** [Enter City, State, ZIP]  
**Contact Name:** [Enter Name]  
**Phone:** [Enter Phone]  
**Email:** [Enter Email]  
**Tax ID/EIN:** [Enter Tax ID]

---

## 1. Scope of Services

### 1.1 Services Description
Service Provider agrees to provide the following services:

[Detailed description of services to be provided]

### 1.2 Deliverables
| Deliverable | Description | Due Date |
|-------------|-------------|----------|
| | | |
| | | |
| | | |
| | | |

### 1.3 Out of Scope
The following items are explicitly excluded from this agreement:
- 
- 
- 

### 1.4 Change Orders
Any changes to the scope of services must be documented in a written Change Order signed by both parties. Change Orders may affect project timeline and compensation.

---

## 2. Compensation

### 2.1 Fee Structure
- [ ] **Fixed Fee:** $[Amount] for complete scope of services
- [ ] **Hourly Rate:** $[Amount] per hour
- [ ] **Daily Rate:** $[Amount] per day
- [ ] **Retainer:** $[Amount] per month

### 2.2 Payment Schedule
| Milestone/Date | Amount | Due Date |
|----------------|--------|----------|
| Upon signing | $ | |
| [Milestone 1] | $ | |
| [Milestone 2] | $ | |
| Upon completion | $ | |
| **TOTAL** | **$** | |

### 2.3 Expenses
- [ ] All expenses included in fee
- [ ] Expenses reimbursed at cost with receipts
- [ ] Expenses reimbursed with [X]% markup
- [ ] Expense cap: $[Amount]

Reimbursable expenses include:
- Travel (air, ground, lodging)
- Materials and supplies
- Third-party services
- Other: _______________

### 2.4 Payment Terms
- Invoices due: Net [30] days
- Late payment fee: [1.5]% per month
- Payment method: [Check/Wire/ACH]

---

## 3. Term and Termination

### 3.1 Term
This Agreement begins on [Start Date] and continues until [End Date], unless terminated earlier.

### 3.2 Termination for Convenience
Either party may terminate this Agreement with [30] days written notice.

### 3.3 Termination for Cause
Either party may terminate immediately if the other party:
- Materially breaches this Agreement and fails to cure within [15] days of notice
- Becomes insolvent or files for bankruptcy
- Engages in illegal or unethical conduct

### 3.4 Effect of Termination
Upon termination:
- Client shall pay for all services rendered through termination date
- Service Provider shall deliver all completed work product
- Both parties shall return confidential information
- Provisions that by their nature should survive shall survive

---

## 4. Intellectual Property

### 4.1 Work Product Ownership
- [ ] **Client owns all work product** upon full payment
- [ ] **Service Provider retains ownership**, grants Client license
- [ ] **Joint ownership** as specified below

### 4.2 Pre-Existing Materials
Service Provider retains ownership of pre-existing materials, tools, and methodologies. Client receives a license to use such materials as incorporated into deliverables.

### 4.3 License Grant
[Specify any license terms if Service Provider retains ownership]

---

## 5. Confidentiality

### 5.1 Definition
"Confidential Information" includes all non-public information disclosed by either party.

### 5.2 Obligations
Each party agrees to:
- Maintain confidentiality of the other's Confidential Information
- Use Confidential Information only for purposes of this Agreement
- Not disclose to third parties without prior written consent

### 5.3 Exceptions
Confidentiality obligations do not apply to information that is:
- Publicly available
- Already known to the receiving party
- Independently developed
- Required to be disclosed by law

### 5.4 Duration
Confidentiality obligations survive termination for [3] years.

---

## 6. Representations and Warranties

### 6.1 Service Provider Represents and Warrants:
- Authority to enter into this Agreement
- Services will be performed professionally and competently
- Deliverables will conform to specifications
- No infringement of third-party intellectual property rights
- Compliance with all applicable laws

### 6.2 Client Represents and Warrants:
- Authority to enter into this Agreement
- Timely provision of required information and approvals
- Payment of all amounts due

### 6.3 Disclaimer
EXCEPT AS EXPRESSLY SET FORTH HEREIN, NEITHER PARTY MAKES ANY WARRANTIES, EXPRESS OR IMPLIED, INCLUDING WARRANTIES OF MERCHANTABILITY OR FITNESS FOR A PARTICULAR PURPOSE.

---

## 7. Limitation of Liability

### 7.1 Cap on Liability
Neither party's total liability shall exceed the total fees paid or payable under this Agreement.

### 7.2 Exclusion of Damages
Neither party shall be liable for indirect, incidental, consequential, special, or punitive damages.

### 7.3 Exceptions
The limitations above do not apply to:
- Breach of confidentiality obligations
- Infringement of intellectual property rights
- Gross negligence or willful misconduct

---

## 8. Indemnification

### 8.1 Service Provider Indemnification
Service Provider shall indemnify Client against claims arising from:
- Service Provider's negligence or misconduct
- Infringement of third-party rights
- Breach of this Agreement

### 8.2 Client Indemnification
Client shall indemnify Service Provider against claims arising from:
- Client's negligence or misconduct
- Client-provided materials
- Breach of this Agreement

---

## 9. Insurance

Service Provider shall maintain:
| Coverage | Minimum Amount |
|----------|----------------|
| General Liability | $ |
| Professional Liability | $ |
| Workers' Compensation | As required by law |

---

## 10. Independent Contractor

Service Provider is an independent contractor, not an employee. Service Provider is responsible for own taxes, insurance, and benefits.

---

## 11. General Provisions

### 11.1 Governing Law
This Agreement is governed by the laws of [State].

### 11.2 Dispute Resolution
Disputes shall be resolved by [mediation/arbitration/litigation] in [Location].

### 11.3 Notices
All notices shall be in writing and sent to the addresses above.

### 11.4 Entire Agreement
This Agreement constitutes the entire understanding between the parties.

### 11.5 Amendment
Amendments must be in writing and signed by both parties.

### 11.6 Assignment
Neither party may assign without prior written consent.

### 11.7 Severability
Invalid provisions shall be modified to be enforceable; remaining provisions continue in effect.

### 11.8 Waiver
Failure to enforce any provision is not a waiver of future enforcement.

---

## Signatures

**CLIENT**

Signature: _________________________

Print Name: _________________________

Title: _________________________

Date: _________________________

**SERVICE PROVIDER**

Signature: _________________________

Print Name: _________________________

Title: _________________________

Date: _________________________

---

<!-- IMPORT INSTRUCTIONS
To import service agreement:
1. Copy the Contract Details table to a CSV file
2. Dates must be YYYY-MM-DD format
3. Amounts should be decimal (no $ symbol)
4. Valid fee_type: fixed | hourly | daily | retainer
5. Valid payment_terms: net_30 | net_15 | net_60 | milestone
6. Valid status: draft | sent | signed | active | completed | terminated
7. Import via ATLVS > Settings > Data Import > Service Contracts
-->
