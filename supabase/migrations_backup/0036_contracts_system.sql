...
CREATE POLICY "Admins can insert contracts"
  ...
  WITH CHECK (
    EXISTS (
      ...
      WHERE id = auth.uid()
      AND organization_id = contracts.organization_id
...
-- Add contract type to vendors table if needed
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'vendors' AND column_name = 'contract_id') THEN
    ALTER TABLE vendors ADD COLUMN contract_id UUID REFERENCES contracts(id);
  END IF;
END $$;
