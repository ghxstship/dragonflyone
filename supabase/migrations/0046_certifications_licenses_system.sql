...
-- Partial indexes with CURRENT_DATE removed (not immutable)
CREATE INDEX idx_crew_certifications_expired ON crew_certifications(expiration_date);
-- CREATE INDEX idx_crew_certifications_expiring ON crew_certifications(expiration_date);
...
CREATE POLICY "Admins can manage certification types"
  ...
  USING (
    EXISTS (
      ...
      WHERE id = auth.uid()
      AND role_in('COMPVSS_ADMIN', 'ATLVS_ADMIN', 'ATLVS_SUPER_ADMIN', 'LEGEND_SUPER_ADMIN')
    )
  );
...
CREATE POLICY "Crew members can view their own certifications"
  ...
  USING (
    ...
    OR EXISTS (
      SELECT 1 FROM platform_users
      WHERE id = auth.uid()
      AND role_in('COMPVSS_ADMIN', 'COMPVSS_TEAM_MEMBER', 'ATLVS_ADMIN', 'LEGEND_SUPER_ADMIN')
    )
  );
...
CREATE POLICY "Users can view related renewals"
  ...
  USING (
    ...
    OR EXISTS (
      SELECT 1 FROM platform_users
      WHERE id = auth.uid()
      AND role_in('COMPVSS_ADMIN', 'ATLVS_ADMIN', 'LEGEND_SUPER_ADMIN')
    )
  );
