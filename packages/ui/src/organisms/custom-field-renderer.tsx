        ...
        return (
          <div className="relative">
            {field.type === "currency" && field.currencyCode && (
              <span className="absolute left-spacing-3 top-1/2 -translate-y-1/2 text-grey-500 font-code text-mono-sm">
                {field.currencyCode}
              </span>
            )}
            <input
            ...
            />
            {field.type === "percent" && (
              <span className="absolute right-spacing-3 top-1/2 -translate-y-1/2 text-grey-500 font-code text-mono-sm">
                %
              </span>
            )}
          </div>
      ...
      case "formula":
        return (
          <div className={clsx(baseInputClass, "bg-surface-secondary flex items-center gap-gap-xs")}>
            <Calculator className="size-4 text-grey-500" />
      ...
      case "reference":
        return (
          <div className="relative">
            ...
            {referenceSearching && (
              <Loader2 className="absolute right-spacing-3 top-1/2 -translate-y-1/2 size-4 animate-spin text-grey-500" />
            )}
          </div>
        );
  ...
  return (
    <div className={clsx("space-y-spacing-1", className)}>
      {showLabel && field.type !== "checkbox" && (
        <label className="flex items-center gap-gap-xs font-code text-mono-sm tracking-wide uppercase text-grey-600">
          {fieldIcons[field.type]}
          <span>{field.label}</span>
          {field.required && <span className="text-error-500">*</span>}
          {permission === "readonly" && <Lock className="size-3 text-grey-400" />}
          {pendingSync && (
      ...
      {field.helpText && field.type !== "checkbox" && !error && (
        <p className="text-grey-500 text-body-xs font-body">{field.helpText}</p>
      )}
    </div>
  );
}
