    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {activity.description && !compact && (
          <p className="text-body-sm text-text-disabled mt-spacing-1 line-clamp-2">
            {activity.description}
          </p>
        )}
        
        <p className="text-body-xs text-text-muted mt-spacing-1">
          ...
          <div className="flex items-center gap-gap-sm">
            {onRefresh && (
              <button
                onClick={onRefresh}
                disabled={loading}
                className="p-spacing-2 text-text-disabled hover:text-text-disabled bg-transparent border-none cursor-pointer transition-colors disabled:opacity-50"
              >
      ...
      <div className="flex flex-col">
        {loading && activities.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-spacing-12 text-center">
            <FileText className="size-12 text-text-secondary mb-spacing-4" />
            <p className="text-body-md text-text-disabled">No activity yet</p>
          ...
          Array.from(groupedActivities.entries()).map(([group, items]) => (
            <div key={group} className="mb-spacing-4">
              <p className="font-code text-mono-xs text-text-disabled uppercase tracking-wider mb-spacing-2 px-spacing-3">
                {group}
              </p>
              <div className="flex flex-col">
                {items.map((activity) => (
